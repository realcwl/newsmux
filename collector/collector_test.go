...
func TestJin10Crawler(t *testing.T) {
	...
	t.Run("[get message from dom element][keynews] html with image", func(t *testing.T) {
		...
		doc, err := goquery.NewDocumentFromReader(strings.NewReader(htmlWithImage))
		if err != nil {
			panic("can't parse html into document")
		}
		elem.DOM = doc.Selection
